...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { RailsInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw RailsInvalidDataException("`$name` is not set")
            is JsonNull -> throw RailsInvalidDataException("`$name` is null")
            else -> throw RailsInvalidDataException("`$name` is invalid, received $this")
        }
